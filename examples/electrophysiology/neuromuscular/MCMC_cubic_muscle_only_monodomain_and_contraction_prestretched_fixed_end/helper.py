...
for j in range(ny):
  for i in range(nx):
    variables.precontraction_elasticity_dirichlet_bc_muscle1[(nz-1)*nx*ny + j*nx + i] = [None,None,0.0,None,None,None]
...
if hasattr(variables, "prestretch_top_traction"):
  ...
  for j in range(ny):
    for i in range(nx):
      variables.prestretch_elasticity_dirichlet_bc_muscle1[(nz-1)*nx*ny + j*nx + i] = [None,None,0.0,None,None,None]
  ...
  #for j in range(ny):
  #  for i in range(nx):
  #    variables.perstretch_elasticity_dirichlet_bc_muscle1[(nz-1)*nx*ny + j*nx + i] = [0.0,0.0,None,None,None,None]
