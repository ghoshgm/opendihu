...
neuron_meshes = {
  ...
  "muscle2Mesh": {
    ...
    "physicalOffset":     [0.0, 0.0, variables.muscle1_extent[2] + variables.start_tendon_length],
    "logKey":             "muscle2",
    "inputMeshIsGlobal":  True,
    "nRanks":             n_ranks
  },
...
config = {
  ...
  "connectedSlots": [
   ...
   "MultipleCoupling": {
     ...
     "Term1": {
       "PrescribedValues": {
         "meshName":               ["muscle1Mesh"],      # reference to the fibers mesh
     ...
     "Term2": {  
       "MuscleContractionSolver": {
         ...
         "HyperelasticitySolver": {
           ...
           "dirichletBoundaryConditions": variables.precontraction_elasticity_dirichlet_bc_muscle1,   # the initial Dirichlet boundary conditions that define values for displacements u and velocity v
           "neumannBoundaryConditions":   variables.precontraction_elasticity_neumann_bc_muscle1,     # Neumann boundary conditions that define traction forces on surfaces of elements
           ...
           "constantBodyForce":           variables.precontraction_constant_body_force_muscle1,       # a constant force that acts on the whole body, e.g. for gravity
     ...
     "Term3": {
       "HyperelasticitySolver": {
         ...
         # boundary and initial conditions
         "dirichletBoundaryConditions": variables.prestretch_elasticity_dirichlet_bc_muscle1,   # the initial Dirichlet boundary conditions that define values for displacements u and velocity v
         "neumannBoundaryConditions":   variables.prestretch_elasticity_neumann_bc_muscle1,     # Neumann boundary conditions that define traction forces on surfaces of elements
         "divideNeumannBoundaryConditionValuesByTotalArea": True,            # if the given Neumann boundary condition values under "neumannBoundaryConditions" are total forces instead of surface loads and therefore should be scaled by the surface area of all elements where Neumann BC are applied
         "updateDirichletBoundaryConditionsFunction": None,                  # function that updates the dirichlet BCs while the simulation is running
         "updateDirichletBoundaryConditionsFunctionCallInterval": 1,         # every which step the update function should be called, 1 means every time step
         ...
         "initialValuesVelocities":     [[0.0,0.0,0.0] for _ in range(variables.n_points_global)],     # the initial values for the velocities, vector of values for every node [[node1-x,y,z], [node2-x,y,z], ...]
         "extrapolateInitialGuess":     True,                                # if the initial values for the dynamic nonlinear problem should be computed by extrapolating the previous displacements and velocities
         "constantBodyForce":           variables.prestretch_constant_body_force_muscle1,       # a constant force that acts on the whole body, e.g. for gravity
     ...
     "Term4": {
       "PrescribedValues": {
         "meshName":               ["muscle2Mesh"],      # reference to the fibers mesh
         "numberTimeSteps":        1,             # number of timesteps to call the callback functions subsequently, this is usually 1 for prescribed values, because it is enough to set the reaction term only once per time step
         "timeStepOutputInterval": 20,            # if the time step should be written to console, a value > 10 produces no output
         "slotNames":              "",
     ...
     "Term5": {  
       "MuscleContractionSolver": {
         ...
         "HyperelasticitySolver": {
           ...
           # boundary and initial conditions
           "dirichletBoundaryConditions": variables.precontraction_elasticity_dirichlet_bc_muscle2,   # the initial Dirichlet boundary conditions that define values for displacements u and velocity v
           "neumannBoundaryConditions":   variables.precontraction_elasticity_neumann_bc_muscle2,     # Neumann boundary conditions that define traction forces on surfaces of elements
           "divideNeumannBoundaryConditionValuesByTotalArea": True,            # if the given Neumann boundary condition values under "neumannBoundaryConditions" are total forces instead of surface loads and therefore should be scaled by the surface area of all elements where Neumann BC are applied
           "updateDirichletBoundaryConditionsFunction": None,                  # function that updates the dirichlet BCs while the simulation is running
           "updateDirichletBoundaryConditionsFunctionCallInterval": 1,         # every which step the update function should be called, 1 means every time step
           ...
           "initialValuesVelocities":     [[0.0,0.0,0.0] for _ in range(variables.n_points_global)],     # the initial values for the velocities, vector of values for every node [[node1-x,y,z], [node2-x,y,z], ...]
           "extrapolateInitialGuess":     True,                                # if the initial values for the dynamic nonlinear problem should be computed by extrapolating the previous displacements and velocities
           "constantBodyForce":           variables.precontraction_constant_body_force_muscle2,       # a constant force that acts on the whole body, e.g. for gravity
     ...
     "Term6": {
       "HyperelasticitySolver": {
         ...
         # boundary and initial conditions
         "dirichletBoundaryConditions": variables.prestretch_elasticity_dirichlet_bc_muscle2,   # the initial Dirichlet boundary conditions that define values for displacements u and velocity v
         "neumannBoundaryConditions":   variables.prestretch_elasticity_neumann_bc_muscle2,     # Neumann boundary conditions that define traction forces on surfaces of elements
         "divideNeumannBoundaryConditionValuesByTotalArea": True,            # if the given Neumann boundary condition values under "neumannBoundaryConditions" are total forces instead of surface loads and therefore should be scaled by the surface area of all elements where Neumann BC are applied
         "updateDirichletBoundaryConditionsFunction": None,                  # function that updates the dirichlet BCs while the simulation is running
         "updateDirichletBoundaryConditionsFunctionCallInterval": 1,         # every which step the update function should be called, 1 means every time step
         ...
         "initialValuesVelocities":     [[0.0,0.0,0.0] for _ in range(variables.n_points_global)],     # the initial values for the velocities, vector of values for every node [[node1-x,y,z], [node2-x,y,z], ...]
         "extrapolateInitialGuess":     True,                                # if the initial values for the dynamic nonlinear problem should be computed by extrapolating the previous displacements and velocities
         "constantBodyForce":           variables.prestretch_constant_body_force_muscle2,       # a constant force that acts on the whole body, e.g. for gravity
